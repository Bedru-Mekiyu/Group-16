...
export const API_BASE_URL = (import.meta.env.VITE_API_BASE_URL || 'https://group-16-production.up.railway.app').replace(/\/$/, '');
...
export const AuthApi = {
  signin: (body: SignInBody) =>
    apiFetch<{ token: string; user: any }>('/api/auth/signin', { method: 'POST', body: JSON.stringify(body) }),

  register: (body: RegisterBody) =>
    apiFetch('/api/auth/register', { method: 'POST', body: JSON.stringify(body) }),

  activate: (body: ActivateBody) =>
    apiFetch('/api/auth/activate', { method: 'POST', body: JSON.stringify(body) }),
